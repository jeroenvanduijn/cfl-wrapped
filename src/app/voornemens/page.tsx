...
export default function VoornemensPage() {
  ...
  const generateStoryCanvas = async (voornemen: Response, gradientIndex: number): Promise<HTMLCanvasElement> => {
    ...
    const quoteText = `"${voornemen.voornemen2026 || ""}"`;
    const maxWidth = canvas.width - boxPadding * 2 - 80;
    const words = quoteText.split(" ");
    let line = "";
    let y = boxY + 120;
  ...
  const downloadAllStories = async () => {
    ...
    try {
      for (let i = 0; i < voornemens.length; i++) {
        setCurrentIndex(i);
        const canvas = await generateStoryCanvas(voornemens[i], i);
